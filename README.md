## Background

Before moving into security, I spent two years in IT Help Desk supporting enterprise environments.  
Hands-on experience includes Active Directory, Azure AD, endpoint support, asset management (Atera), phishing awareness training, and legacy system documentation.

This foundation allows me to understand how systems behave in real environments — not just in a lab — before an alert ever fires.

---

## Current Focus

Building toward a Tier 1 SOC role with emphasis on:

- Log analysis & SIEM (Wazuh)
- Threat detection & alert validation
- Network traffic analysis
- Incident triage
- Linux & system enumeration
- Web application security
- Attacker TTPs

## Labs & Writeups

I document hands-on lab work here:  
https://github.com/Kamal-Subba/home-soc-lab

Writeups are structured to reflect real-world analysis and communication — focusing on detection, investigation, and response.

---

Certifications

-    **CompTIA Security+**
  - Issued by: CompTIA
  - [View Credentials](https://www.credly.com/badges/fd762fbf-4283-4ae1-af8a-ac0330c7629f/public_url)
    
-    **CompTIA Network+**
   - Issued by: CompTIA
   - [View Credentials](https://www.credly.com/badges/33d784c3-fd03-48ef-ab8a-5fdab8912b70/public_url)
     
-    **Cybersecurity Black Belt — Coding Dojo**

## Training & Labs

  TryHackMe – Pre Security (SEC0)
  [View Certificate](https://assets.tryhackme.com/certification-certificate/69cd149a437e95c92b6417bd.pdf)
  [Credly](https://www.credly.com/badges/5326593a-ae13-4d37-a2e2-3d5583c1afa2/public_url)

  TryHackMe – Cyber Security 101 (SEC1)
  [View Certificate](https://assets.tryhackme.com/certification-certificate/69d66659e244533ec0851009.pdf)
  [Credly](https://www.credly.com/badges/dc280dfe-cc9f-4380-b34e-7db082cd0373/public_url)


[TryHackMe](https://tryhackme.com/p/Blazerz)
    

Open to entry-level SOC opportunities, feedback, and connecting with others in the field.
